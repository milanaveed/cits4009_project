...
```{r}
# Calculate median for each country and filter the top 20
top_countries <- df %>%
  group_by(Country) %>%
  summarise(median_earnings_country = median(highest_yearly_earnings, na.rm = TRUE)) %>%
  arrange(-median_earnings_country) %>%
  head(20) %>%
  pull(Country)

# Filter the original data frame to only include these top 20 countries
df_filtered <- df %>% filter(Country %in% top_countries)

# Create the boxplot
ggplot(df_filtered, aes(x = reorder(Country, median_earnings_country), y = highest_yearly_earnings)) +
  geom_boxplot() +
  coord_flip() +
...
mp2<-mp+geom_point(aes(x=visit.x,y=visit.y,size=df$highest_yearly_earnings),color="darkorange")+scale_size(range=c(2,9))+ggtitle("plot name")+theme(plot.title=element_text(size=25,color="red",face="italic"))+theme_grey(base_size = 32)  
