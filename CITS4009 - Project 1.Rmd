...
# Select the first 10 unique country names
selected_countries <- unique_countries[1:25]

# Get all rows of the top 10 countries with the largest amount of famous channels
...
# Create a subset containing only two columns for later plotting
df1 <- subset(df1, select = c("Country", "category.fix"))

# Sort the data frame
df1 <- df1 %>% arrange(desc(df_grouped$count))
...
# # Select the first 10 unique country names
# selected_countries <- unique_countries[1:10]
#
# # Get all rows of the top 10 countries with the largest amount of famous channels
# top_10_country <- subset(df_median_earnings_country_des, df_median_earnings_country_des$Country %in% selected_countries)



ggplot(df_grouped, aes(x = category.fix, y = Country)) +
  geom_count(aes(size = df_grouped$count, group = category.fix)) +
