...
top_10_country <- subset(df1, df1$Country %in% selected_countries)

# Group country and category, then count the occurrences of each combination
df_grouped <- top_10_country %>% 
...
ggplot(df_grouped, aes(x = Abbreviation, y = category.fix)) +
  geom_count(aes(size = df_grouped$count, group = category.fix), color=my_color, alpha=0.6) +
