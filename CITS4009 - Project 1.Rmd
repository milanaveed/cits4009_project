...
### Filtering data
TODO:
- take out the NA values in category?
...
# Plot the bar chart
ggplot(categoryf) +
  geom_bar(aes(x = category, y = count), stat = "identity", fill = "gray") +
  coord_flip() +
...
p1 <- ggplot(df, aes(x = lowest_monthly_earnings)) +
  ...
  annotate("text", x = 100, y = 0.5, label = paste("The lowest monthly earning peaks"," at around 25000", sep="\n"))+
...
p2 <- ggplot(df, aes(x = lowest_monthly_earnings)) +
  ...
  geom_density(color=my_color)+
  scale_x_continuous(breaks=c(200000,500000), limits = c(0, 2.1e+05), oob = scales::oob_squish)+
  scale_y_continuous(limits = c(0, 0.00004), oob = scales::oob_squish)+
  annotate("text", x = 100000, y = 2e-05, label = paste("Most of the distribution is concentrated", "at the low end: less than 100000", sep="\n"))+
...
# Plot the bar chart
ggplot(countryf) +
  geom_bar(aes(x = country, y = count), stat = "identity", fill = "gray") +
  coord_flip() +
